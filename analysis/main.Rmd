...
project_name <- "fs20c4_steinschlagrisiko" # adapt!
...
### `probability_car_hit.csv`

| hour | events | cars_danger_zone_per_hour | time_danger_zone_seconds | exp_val_car | exp_val_rockfall | total_exp_val |
|------|--------|-------------------------------------------------------------------------------------------------------|
| 0    | 2      | 114.66559                 | 28.907292                | 0.008029803 | 0.02020202       | 0.00016221825 |
| 1    | 2      | 53.51061                  | 13.490069                | 0.003747242 | 0.02020202       | 0.00007570185 |
| 2    | 3      | 38.22186                  | 9.635764                 | 0.002676601 | 0.03030303       | 0.00008110912 |
...
library(MASS) # advanced statistics
...
dplyr::select(data01, c(mass, speed, energy_kJ)) %>% 
  summarise_each(funs(var, sd, IQR))

dplyr::select(data02, c(mass, speed, energy_kJ)) %>% 
  summarise_each(funs(var, sd, IQR))
```
...
p3 <- ggplot(data01, aes(sample = mass)) + 
  geom_abline(slope = 1, intercept = 0, color = "#d4d6dd") +
...
p1 <- ggplot(data = data01, mapping = aes(x = mass, y = speed)) +
  ...
  labs(title = "Mass/Speed Dataset 01") +
  theme_dviz_open()
...
p2 <- ggplot(data = data02, mapping = aes(x = mass, y = speed)) +
  ...
  labs(title = "Mass/Speed Dataset 02") +
  theme_dviz_open()
...
p3 <- ggplot(data = data01, mapping = aes(x = mass, y = energy_kJ)) +
  ...
  labs(title = "Mass/Energy Dataset 01") +
  theme_dviz_open()
...
p4 <- ggplot(data = data02, mapping = aes(x = mass, y = energy_kJ)) +
  ...
  labs(title = "Mass/Energy Dataset 02") +
  theme_dviz_open()
...
p1 <- ggplot(data = df1_df2, aes(x = speed, y = mass, size = mass)) +
  ...
  labs(title = "Mass/Speed all boulders") +
  theme_dviz_grid(12) +
  theme(
...
p2 <- ggplot(data = df1_df2, aes(x = energy_kJ, y = mass, size = mass)) +
  ...
  labs(title = "Mass/Energy all boulders") +
  theme_dviz_grid(12) +
  theme(
...
rockfalls_hour_hightlight <- rockfalls_hour %>%
  mutate(tohighlight = ifelse(n %in% c(9, 8, 7), "yes", "no"))

# Total rockfalls per day
total_rockfalls <- sum(rockfalls_hour$n)

p <- ggplot(data = rockfalls_hour_hightlight, aes(x = time, y = n, fill = tohighlight)) +
  geom_col(fill = "#22577a", alpha = 0.9) +
  scale_y_continuous(name = "Count",
                     expand = c(0, 0),
                     breaks = c(seq(0, 8, 2))) +
  geom_col(aes(fill = tohighlight)) +
  scale_fill_manual(
  ...
  scale_x_time(
    ...
    position = "bottom",
    breaks = unique(rockfalls_hour_hightlight$time),
...
p <- ggplot(data = exp_val_car_hit) +
  ...
  scale_color_manual(name = "",
                   values = c("Rockfall event" = "#f2ac18",
                              "Car driving through" = "#22577a",
                              "Boulder hit" = "#df3340"),
                   breaks = c("Rockfall event",
                              "Car driving through",
                              "Boulder hit")) +
...
# Showing final probability result
...
# Create a .csv file as output file for replication and further analysis
# write_csv(exp_val_car_hit, "output/probability_car_hit.csv") # uncomment to create the file
...
boulder_event <- function(speed, mass) {
  mass_in_net
  num_boulder_through_net
  broken <- FALSE
  ...
  if (broken) {
    num_boulder_through_net <-  num_boulder_through_net + 1
...
car_hitting <- function(event_time, day_minutes) {
  ...
  if (hit_number <= 0.04835726) {
    car_hit_mean <- car_hit_mean + 1
  }
  if (hit_number <= 0.104387442) {
...
monte_carlo_rockfall <- function(num_of_years, clearing_time_hour) {
  ...
  for (year in 1:num_of_years) {
    ...
    mass_in_net <- 0
    now <- 0
    time_to_next_event_data01 <- 3732
    time_to_next_event_data02 <- 3892
    while (now < year_minutes) {
      if (time_to_next_event_data01 < time_to_next_event_data02) {
      net_clearing_process(now, time_to_next_event_data01, clearing_time_minute, day_minutes)
      time_to_next_event_data02 <- time_to_next_event_data02 - time_to_next_event_data01
      now <- now + time_to_next_event_data01
      time_to_next_event_data01 <- 3746
      speed_1 <- rnorm(1, mean = norm_fit_speed_data01$estimate[1], sd = norm_fit_speed_data01$estimate[2])
      mass_1 <- rlnorm(1, meanlog = lognorm_fit_mass_data01$estimate[1], sdlog = lognorm_fit_mass_data01$estimate[2])
      broken <- boulder_event(speed_1, mass_1)
      if (broken) {
        ...
        net_clearing_process(now, time_to_next_event_data02, clearing_time_minute, day_minutes)
        time_to_next_event_data01 <- time_to_next_event_data01 - time_to_next_event_data02
        now <- now + time_to_next_event_data02
        time_to_next_event_data02 <- 3864
        speed_2 <- rnorm(1, mean = norm_fit_speed_data02$estimate[1], sd = norm_fit_speed_data02$estimate[2])
        mass_2 <- rexp(1, rate = expo_fit_mass_data02$estimate[1])
        broken <- boulder_event(speed_2, mass_2)
        if (broken){
          car_hitting(now, day_minutes)
          break
        }
  ...
  prob_boulder_through_net_mc <- num_boulder_through_net / num_of_years
  prob_car_hit_stat_mc <- car_hit_stat / num_of_years
  prob_car_hit_mean_mc <- car_hit_mean / num_of_years
  prob_car_hit_max_mc <- car_hit_max / num_of_years
  
  df <- data.frame(
                   'num_boulder_through_net' = num_boulder_through_net,
                   'prob_boulder_through_net_mc' = prob_boulder_through_net_mc,
                   'car_hit_stat' = car_hit_stat,
                   'prob_car_hit_stat_mc' = prob_car_hit_stat_mc,
                   'car_hit_mean' = car_hit_mean,
                   'prob_car_hit_mean_mc' = prob_car_hit_mean_mc,
...
monte_carlo_rockfall(num_of_years = 100, clearing_time_hour = 8)
```
...
```{r linting, echo=TRUE, message=FALSE, warning=FALSE, results='hide'}
