...
# Traffic_density data is scaled to 217.0795 percent and needs to be scaled down to 100 percent
data_traffic <- mutate(data_traffic, pct_traffic = pct_traffic / 2.170795)
...
(rockfalls_ev <- mutate(rockfalls_hour, expected_value = n / 0.99 / 100))

# Let´s double check our results
sum(rockfalls_ev$expected_value) # must be 1
...
# Total time (in seconds) one car stays in the danger area
time_car_da = 16.66 / 4.2
...
(car_per_hour <- mutate(data_traffic,
                       cars_passing = pct_traffic * 12,
                       time_in_danger_seconds = cars_passing * time_car_da,
                       exp_val_car_per_hour = time_in_danger_seconds / 3600
                       ))
...
# (expected_value_car <- mutate(rockfalls_ev, time, n, expected_value,
#                           car_passing_hour = car_per_hour$cars_passing,
#                           t_car_in_danger = car_per_hour$time_in_danger_seconds,
#                           exp_car_per_hour = car_per_hour$exp_val_car_per_hour,
#                           t_exp_per_hour = expected_value * car_per_hour$exp_val_car_per_hour))
# # 
